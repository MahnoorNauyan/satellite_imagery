...
# # from convert_box_to_poly import convert
...
# #     images_folder = 'output2/predict/polygons/images'
# #     labels_folder = 'output2/predict/polygons/labels'
...
# #                 count += 1
# #                 #st.write(f"House size for polygon {i+1}: {house_size} square feet")
# #                 #st.write(f"Property Tax for polygon {i+1}: Rs.{tax}")
...
def draw_polygons_on_image(image_path, label_path):
    ...
    for i, vertices in enumerate(polygons):
        cv2.polylines(image, [np.array(vertices)], True, (255, 0, 0), thickness=2)
        # Add annotation
        cv2.putText(image, f"House {i}", (vertices[0][0], vertices[0][1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
...
def double_storey_tax():
    # convert()   #Uncomment this to perform bounding box conversion to polygon
    images_folder = 'imagefordoublestorey'
    labels_folder = 'ilabelfordoublestorey'
    ...
    if os.path.exists(label_path):
        ...
        for i, vertices in enumerate(polygons):
            ...
            st.write(f"**tax:**{i} Rs.{tax_double}")
            total_area += house_size
            count += 1
        st.write(f"**Latitude:** {latitude}")
        st.write(f"**Longitude:** {longitude}")
